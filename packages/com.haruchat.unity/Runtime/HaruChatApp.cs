...
namespace HaruChat.Unity
{
    ...
    public sealed class HaruChatApp : MonoBehaviour
    {
        ...
        private async Task UpdateDiagnostics()
        {
            ...
            var contextText = context == null ? "—" : context.UsedTokens + " / " + context.PromptBudgetTokens + " (출력 예약 " + context.OutputReserveTokens + ", 여유 " + context.RemainingTokens + ")" + (context.ActualTokenizerPromptTokens.HasValue ? " · tokenizer 실측" : " · 추정") + (context.MemoryEstimatedTokens > 0 ? " · 기억 " + context.MemoryEstimatedTokens : string.Empty) + (context.SummaryEstimatedTokens > 0 ? " · 요약 " + context.SummaryEstimatedTokens : string.Empty) + (context.ConversationEstimatedTokens > 0 ? " · 대화 " + context.ConversationEstimatedTokens : string.Empty) + (context.CompactedCompletedTurns > 0 ? " · 압축 " + context.CompactedCompletedTurns + " turn" : string.Empty) + (context.ExcludedCompletedTurns > 0 ? " · 이전 turn " + context.ExcludedCompletedTurns + "개 제외" : string.Empty);
