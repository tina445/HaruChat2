...
namespace HaruChat.Runtime.Characters
{
    public sealed class CharacterChatService : IAsyncDisposable
    {
            ...
            try
            {
                ...
                if (_memoryOptions.Enabled && _memoryRetriever != null)
                {
                    try
                    {
                        ...
                        var directLimit = !string.IsNullOrWhiteSpace(summary) && remainingItems > 1 ? remainingItems - 1 : remainingItems;
                        if (directLimit > 0) recalled.AddRange(await _memoryRetriever.SearchAsync(new MemoryQuery(_character.Id, input, directLimit), linked.Token).ConfigureAwait(false));
                        remainingItems = _compiler.MemoryPolicy.MaximumItems - recalled.Count;
                        if (remainingItems > 0 && !string.IsNullOrWhiteSpace(summary))
                        {
                            var knownIds = new HashSet<string>(StringComparer.Ordinal);
                            foreach (var recalledItem in recalled) knownIds.Add(recalledItem.MemoryId);
                            foreach (var item in await _memoryRetriever.SearchAsync(new MemoryQuery(_character.Id, Trim(summary, 512), remainingItems), linked.Token).ConfigureAwait(false))
                            {
                                if (knownIds.Add(item.MemoryId)) recalled.Add(item);
                                if (recalled.Count >= _compiler.MemoryPolicy.MaximumItems) break;
                            }
                        }
                        memories = recalled;
                    }
                ...
                while (tokenCount.Tokens > promptBudget && _conversation.LiveCompletedTurns > _compactionPolicy.RetainedCompletedTurns)
                {
                    _conversation.DiscardOldestLiveTurns(1, _compactionPolicy.RetainedCompletedTurns);
                    plan = _compiler.CompilePlan(_character, _conversation, input, promptBudget, memories: memories);
                    tokenCount = await CountPromptTokensAsync(plan, linked.Token).ConfigureAwait(false);
                }
                _lastEstimatedPromptTokens = plan.EstimatedPromptTokens; _lastActualPromptTokens = tokenCount.Tokens; _lastExcludedTurns = plan.ExcludedCompletedTurns; _lastSummaryTokens = plan.SummaryEstimatedTokens; _lastCompactedTurns = plan.CompactedCompletedTurns; _lastMemoryTokens = plan.MemoryEstimatedTokens; _lastConversationTokens = plan.ConversationEstimatedTokens;
        ...
        public async Task<ContextWindowStatus> GetContextWindowStatusAsync(CancellationToken cancellationToken)
        {
            ...
            return new ContextWindowStatus(_contextBudget, _maximumOutputTokens, _lastEstimatedPromptTokens, usage.PromptTokens, usage.GeneratedTokens, _lastExcludedTurns, _lastSummaryTokens, _lastCompactedTurns, _lastActualPromptTokens, _lastMemoryTokens, _lastConversationTokens);
        }
        /// <summary>
        ...
        public async Task ReplaceSessionAsync(CharacterDefinition character, IModelSession session, int contextBudget, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                _character = character; _session = session; _contextBudget = contextBudget; _compressor = session is ITokenCountingModelSession ? new ModelConversationCompressor(session, _compactionPolicy.MaximumSummaryOutputTokens) : null; _conversation.Reset(); BeginMemorySession();
            }
            finally { _operationLock.Release(); }
        }
        ...
        private async Task PersistCompletedTurnAsync(string input, string response, CancellationToken cancellationToken)
        {
            ...
            try
            {
                var summary = _conversation.CreateSessionHandoff(900);
                if (!string.IsNullOrWhiteSpace(summary) && !MemoryPrivacy.IsSensitive(summary)) await _memoryStore.UpsertSessionAsync(new MemorySession(_memorySessionId, _character.Id, summary, _memorySessionCreated, now, expires), cancellationToken).ConfigureAwait(false);
