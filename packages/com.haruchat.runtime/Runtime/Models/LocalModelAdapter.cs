...
namespace HaruChat.Runtime.Models
{
    ...
    public sealed class LocalModelAdapter : IModelAdapter
    {
        ...
        public async Task<IModelSession> CreateSessionAsync(ModelSessionOptions options, CancellationToken ct)
        {
            ...
            try { var runtimeMetadata = await _backend.GetRuntimeMetadataAsync(runtime.Value, ct).ConfigureAwait(false); Ensure(runtimeMetadata); var model = await _backend.LoadModelAsync(runtime.Value, new LocalModelLoadOptions(_config.ModelPath), ct).ConfigureAwait(false); Ensure(model); try { var context = await _backend.CreateContextAsync(model.Value, new LocalContextOptions(_profile.ResolveContextWindow(_config), options.BatchSize), ct).ConfigureAwait(false); Ensure(context); var metadata = await _backend.GetModelMetadataAsync(model.Value, ct).ConfigureAwait(false); return new Session(_backend, runtime.Value, model.Value, context.Value, _profile, _config, watch.Elapsed, runtimeMetadata.Value, metadata.IsSuccess ? metadata.Value : null); } catch { await _backend.UnloadModelAsync(model.Value, CancellationToken.None).ConfigureAwait(false); throw; } }
        ...
        {
            private readonly ILocalModelBackend _backend; private readonly LocalRuntimeHandle _runtime; private readonly LocalModelHandle _model; private readonly LocalContextHandle _context; private readonly ModelProfile _profile; private readonly ModelConfig _config; private readonly TimeSpan _loadDuration; private readonly LocalRuntimeMetadata _runtimeMetadata; private readonly LocalModelMetadata? _metadata; private readonly SemaphoreSlim _lifetime = new SemaphoreSlim(1, 1); private bool _disposed; private ModelUsage _usage = new ModelUsage(null, null, null); private ModelDiagnostics _diagnostics;
            public Session(ILocalModelBackend backend, LocalRuntimeHandle runtime, LocalModelHandle model, LocalContextHandle context, ModelProfile profile, ModelConfig config, TimeSpan loadDuration, LocalRuntimeMetadata runtimeMetadata, LocalModelMetadata? metadata)
            { _backend = backend; _runtime = runtime; _model = model; _context = context; _profile = profile; _config = config; _loadDuration = loadDuration; _runtimeMetadata = runtimeMetadata; _metadata = metadata; _diagnostics = new ModelDiagnostics(runtimeMetadata.BackendName, runtimeMetadata.BackendName.IndexOf("metal", StringComparison.OrdinalIgnoreCase) >= 0 ? true : (bool?)null, metadata?.ContextWindowTokens ?? profile.ContextWindowTokens, loadDuration, null, null, null); }
            public async IAsyncEnumerable<ModelEvent> GenerateAsync(ModelRequest request, [System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken ct)
            {
                ...
                try
                {
                    ...
                    var options = request.Generation ?? _profile.ResolveGeneration(_config); var start = await _backend.StartGenerationAsync(_context, new LocalGenerationOptions(Serialize(request.Messages), options.MaximumOutputTokens, options.Temperature, options.TopK, options.TopP, options.Seed.HasValue ? unchecked((uint)options.Seed.Value) : unchecked((uint)Guid.NewGuid().GetHashCode())), ct).ConfigureAwait(false); Ensure(start); job = start.Value; started = true; pump = PumpAsync(job, queue, pumpCancellation.Token);
            ...
            private async Task PumpAsync(LocalGenerationHandle job, EventQueue queue, CancellationToken cancellationToken)
            {
                ...
                try { while (true) { cancellationToken.ThrowIfCancellationRequested(); var batch = await _backend.PollEventsAsync(job, 32, cancellationToken).ConfigureAwait(false); Ensure(batch); if (batch.Value.Events.Count == 0) { await Task.Delay(8, cancellationToken).ConfigureAwait(false); continue; } foreach (var raw in batch.Value.Events) { switch (raw.Kind) { case LocalBackendEventKind.Token: var text = Decode(decoder, raw.Payload, false); if (text.Length > 0 && firstToken == null) firstToken = watch.Elapsed; if (!await WriteTextAsync(queue, stops.Push(text), stops.Stopped, cancellationToken).ConfigureAwait(false)) return; break; case LocalBackendEventKind.Metrics: _usage = new ModelUsage(raw.Metrics.PromptTokenCount, raw.Metrics.GeneratedTokenCount, raw.Metrics.Elapsed); _diagnostics = new ModelDiagnostics(_runtimeMetadata.BackendName, _runtimeMetadata.BackendName.IndexOf("metal", StringComparison.OrdinalIgnoreCase) >= 0 ? true : (bool?)null, _metadata?.ContextWindowTokens ?? _profile.ContextWindowTokens, _loadDuration, firstToken, Rate(raw.Metrics.PromptTokenCount, raw.Metrics.Elapsed), Rate(raw.Metrics.GeneratedTokenCount, raw.Metrics.Elapsed)); await queue.WriteAsync(ModelEvent.UsageSnapshot(_usage), cancellationToken).ConfigureAwait(false); break; case LocalBackendEventKind.Completed: if (!await WriteTextAsync(queue, stops.Push(Decode(decoder, ReadOnlyMemory<byte>.Empty, true)) + stops.Flush(), stops.Stopped, cancellationToken).ConfigureAwait(false)) return; await queue.WriteAsync(ModelEvent.Completed(), cancellationToken).ConfigureAwait(false); queue.Complete(); return; case LocalBackendEventKind.Cancelled: queue.Complete(new OperationCanceledException()); return; case LocalBackendEventKind.Error: await queue.WriteAsync(ModelEvent.ErrorEvent(new ModelError(Map(raw.Error.Code), raw.Error.Message)), cancellationToken).ConfigureAwait(false); queue.Complete(); return; default: queue.Complete(new ModelOperationException(ModelErrorCode.BackendFailure, "Unknown local event.")); return; } } } }
